...
namespace ETModel
{
    public class AoiComponent : Component
    {
        ...
        public AoiNode UpdateNode(AoiNode node, Vector2 area, float x, float y)
        {
            ...
            node.AoiInfo.OldMovesSet = node.AoiInfo.MovesSet.Select(d => d).ToHashSet();
            ...
            node.AoiInfo.Enters = node.AoiInfo.MovesSet.Except(node.AoiInfo.OldMovesSet).ToHashSet();

            node.AoiInfo.Leaves = node.AoiInfo.OldMovesSet.Except(node.AoiInfo.MovesSet).ToHashSet();
